        ...
        <form action="{{ route('admin.documents.update', $document) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="title_th" class="form-control @error('title_th') is-invalid @enderror" value="{{ old('title_th', $document->title_th) }}" required>
                    @error('title_th') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6">
                    <label class="form-label">หัวข้อ (อังกฤษ)</label>
                    <input type="text" name="title_en" class="form-control @error('title_en') is-invalid @enderror" value="{{ old('title_en', $document->title_en) }}">
                ...
                <div class="col-md-6">
                    ...
                    <select name="year_id" class="form-select @error('year_id') is-invalid @enderror">
                        ...
                        @foreach($years as $year)
                            <option value="{{ $year->id }}" {{ old('year_id', $document->year_id) == $year->id ? 'selected' : '' }}>
                                {{ $year->year }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="text-end">
                <button type="submit" class="btn btn-primary px-4"><i class="bi bi-save"></i> อัปเดตเอกสาร</button>
            </div>
        </form>
    </div>
</div>
